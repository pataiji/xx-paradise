...
class XxParadise {
    ...
    Array.prototype.forEach.call(imgs, (img) => {
      if (img.src.match(new RegExp(this.config.activeSrcRegexp))) {
        ...
        if (width > 0 && height > 0) {
          ...
          img.src = '';
          img.src = this.buildImageUrl(width, height, fingerprint);
        }
      }
    });
  }
  ...
  buildImageUrl(width, height, fingerprint) {
    ...
    return '//' + this.config.endpointDomain + '/' + parameter + '/' + fingerprint;
  }
...
Config.read().then((config) => {
  if (config.status === 'enabled' && document.domain.match(new RegExp(config.activeDomainRegexp))) {
